...
| 5     | ✅ done  | 3D globe via `expo-gl` + `three.js` + `expo-three`. `GLView` renders a Three.js scene: textured earth sphere (NASA Blue Marble loaded from GitHub CDN), `PanResponder` drag-to-spin, auto-rotation when idle, red pins at each stream's lat/lng, raycaster tap-to-join. Metro blocklist added for `three/examples` + `three/src` to protect Ben's EMFILE limit. Packages: `expo-gl`, `expo-location`, `expo-three`, `three`, `@types/three`. |
| 6     | next     | Dashboard with stream layer arming (audio/video/overlays/chat — exact "layer" semantics TBD)                                                                                                                                                                                                                                                                                                                                                                                           |
