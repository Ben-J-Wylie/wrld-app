...
| **SP2** | Ben | `design` | **Preview parity.** Audio is now **metered in PREVIEW too**, not just live (2026-06-17): `useMediasoup.startPreviewMeter` spins a throwaway local `RTCPeerConnection` holding the mic track so `getStats().media-source.audioLevel` reports before go-live (no server, no new dep) — torn down on go-live (the producer poll takes over), on a source change, and in cleanup. temp stays idle (no phone sensor); `screen` not in the rail (no capture). | ✅ done 2026-06-17 (audio live in preview + live + viewer) |
| **SP3** | Ben | `design` | **Complete the stream rail: chat · profile as live views** (`chat` → `SourceChatLog`; `profile` → `SourceIdentityCard`). **Both rails unified** on the shared `SOURCE_RAIL_ORDER` (full 12-source suite, dashboard order, horizontal + scrollable) — `loc` is now PRESENT on the stream rail too, as an honest idle until the SP5 relay; clips shows the full suite with idle for what a clip can't replay. | ✅ done 2026-06-16 (chat + profile + unified rail; loc idle on stream until SP5) |
...
| **SP6** | Ben + Aaron | both | **Persist/save parity audit + screen source.** Confirm every armed source writes a track AND promotes into a saved clip (sensors/chat/location ✓ via item 2 + C6; audit torch on-toggle emit/record). **Screen** source end-to-end (capture/produce + live view + record) is the largest remaining gap — likely its own slice. | **SP6a audit done 2026-06-17** (see note below); **SP6b screen DEFERRED to v0.3**; **torch control ✅ done (Ben, 2026-06-17)** — `TorchVisualizer` gained `onToggle`; the broadcaster holds `torchOn` + emits `sendTelemetry({kind:'torch',on})` (viewers light up via the fan-out; recorded when armed). Signaled on/off channel — the real device LED stays a separate device spike. |
