...
is the PB4 per-segment work below (Ben's lane).
...
- **Effect:** marking the second half public when the whole clip is private now correctly
  leaves the first half private (subtract). Combined with **Aaron's head/tail footage-window
  fix above**, per-segment behaviour is correct **with the current scab-in UI** — no redesign
...
> The fix: tile on `(planet, t, z, x, y)` — a top-level **planet** partition (Earth/Venus/Haven/…, viewer on one at a time) × time-tile × the existing slippy geo-tile (`tiles.ts`) — with
