...
export function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // No auto-resume of an abandoned sign-up here: an unverified sign-up is
      // indistinguishable from an unknown email at sign-in. Point a no-account user
      // to Sign up, which resumes/finishes the sign-up (with its own resend).
      Alert.alert(
        'Sign in failed',
        clerkError(err, "We couldn't find an account with those details. Tap Sign up to create one."),
      )
    } finally {
      setLoading(false)
    }
