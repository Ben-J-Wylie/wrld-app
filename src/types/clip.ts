...
export function fromSavedClip(c: SavedClip): CanonicalClip {
  return {
    ...
    host: null,
    axes: {
      title: c.name && c.name !== 'Untitled clip' ? c.name : null,
      tags: [],
      visibility: 'public',
      identity: c.attributed ? 'shown' : 'anon',
      precision: precisionToResolved((c.locDisplayPrecision as SegSettings['precision']) ?? undefined),
      sources: c.sources,
      keep: c.saved ? 'kept' : 'reapable',
    },
    lat: null,
    lng: null,
